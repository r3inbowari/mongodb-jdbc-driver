...
//        }
//        return null;
        // TODO r3inbowari date 遍历 为了适配davinci这里的getDate重写了一个不相关 对所有时间格式进行格式化
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");

        if (expandResultSet && current instanceof Map) {
            int current_size = ((Map) current).size();

            for (int i = 0; i < current_size; i++) {
                Object obj = ((Map) current).get(metaColumnsNames.get(i));
                if (obj instanceof Date) {
                    Date da = (Date) obj;
                    metaJavaTypes.set(i, Types.TIMESTAMP);
                    ((Map) current).put(metaColumnsNames.get(i), df.format(da));

                }
                if (obj instanceof java.util.Date) {
                    Date da = new Date(((java.util.Date) obj).getTime());
                    metaJavaTypes.set(i, Types.TIMESTAMP);
                    ((Map) current).put(metaColumnsNames.get(i), df.format(da));
                }
            }
